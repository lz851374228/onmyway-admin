...
<script lang="ts" setup>
  ...
  import { formItemsForEach, generateKey, formatRules, removeAttrs } from '../../utils';
  ...
  import { updateTemplate } from '/@/api/form/template';
  ...
  const handleListPushDrag = (item: IVFormComponent) => {
    ...
    // console.log('handleListPushDrag', formItem);
  ...
  const handleCopy = (
    ...
    const traverse = (schemas: IVFormComponent[]) => {
      ...
      schemas.some((formItem: IVFormComponent, index: number) => {
        console.log('traverse', item, key, formItem);
        if (formItem.key === key) {
          // 判断是不是复制
          isCopy
  ...
  const handleChangePropsTabs = (key: PropsTabKey) => {
    // console.log('handleChangePropsTabs', key);
  ...
  let timer: any = null;
  onMounted(() => {
    const formId = parseInt(route.query?.id as string);
    timer =
      formId &&
      setInterval(async () => {
        let jsonData = cloneDeep(formConfig.value);
        // await formatRules(jsonData.schemas);
        // await removeAttrs(jsonData);
        await updateTemplate({
          id: formId,
          schemas: JSON.stringify(jsonData),
        });
      }, 2000);
  });
  onUnmounted(() => {
